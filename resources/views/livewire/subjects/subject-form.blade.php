    ...
    <x-card title="Materia" shadow separator>
        <x-form wire:submit="save" no-separator>
            <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                ...
                <x-select label="Carrera" icon="o-academic-cap" :options="$this->careers"
                    wire:model.live="data.career_id" />
            </div>
                        ...
                        <div class="overflow-y-auto max-h-64">
                            @foreach ($this->subjects as $subject)
                                <div @class([
                                    ...
                                    'bg-primary/30 text-primary-content font-bold' => in_array($subject->id, $subjectsToStudy),
                    ...
                    <div class="p-4 rounded-lg bg-base-200">
                        ...
                        <div class="overflow-y-auto max-h-64">
                            @foreach ($this->subjects as $subject)
                                <div @class([
                                    'border border-white/10 px-2 py-1 cursor-pointer hover:bg-base-300 transition-colors',
                                    'bg-primary/30 text-primary-content font-bold' => in_array($subject->id, $subjectsToExam),
            ...
            <x-slot:actions>
                @if($data['id'])
                    <x-dropdown label="ELIMINAR" icon="o-trash" class="btn-error btn-outline">
                        <x-menu-item title="Confirmar eliminación" icon="o-trash" wire:click="delete" spinner
                            class="text-error" />
                    </x-dropdown>
                @endif
