...
>
    ...
    <div class="px-2 pb-24 space-y-2 max-w-4xl mx-auto mt-4">
        @if(count($recessEvents) > 0)
            <div class="alert alert-warning shadow-sm border border-warning/30 mb-4 rounded-xl flex items-start gap-3">
                <x-icon name="o-exclamation-triangle" class="w-6 h-6 shrink-0 mt-0.5" />
                <div>
                    <h3 class="font-bold text-sm">Aviso de Receso Lectivo / Feriado</h3>
                    <p class="text-xs opacity-95">Se ha detectado un receso o feriado en la fecha seleccionada:</p>
                    <ul class="list-disc list-inside text-xs mt-1 font-semibold">
                        @foreach($recessEvents as $event)
                            <li>{{ $event['title'] }}</li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="card-body p-0 gap-0">
                    ...
                    <div class="flex items-center justify-between gap-2 p-3 cursor-pointer hover:bg-base-200/50 transition-colors" @click="open = !open">
                        <div class="flex items-center gap-2">
                            ...
                            @if(isset($justifications[$uid]))
                                <span class="tooltip" data-tip="Justificación: {{ $justifications[$uid]['description'] }}">
                                    <span class="badge badge-info badge-sm gap-1 ml-1 text-white py-0.5 px-2">
