...
class InscriptionManager extends Component
{
    ...
    public function inscriptions()
    {
        if ($this->targetUser->hasAnyRole(['admin', 'principal', 'director', 'administrative'])) {
            return Config::where('group', 'inscriptions')->get();
        }

        return Config::where('group', 'inscriptions')
            ->where('value', 'true')
            ->get();
    }
    ...
    public function items(): Collection
    {
        ...
        $inscriptions = Inscription::where('configs_id', $this->inscription_id)
        ...
        if ($this->targetUser->hasRole('student')) {
            $selected = Inscription::where('configs_id', $this->inscription_id)
        ...
        foreach ($subjects as $subject) {
            ...
            // Preserve current selection if it exists in local state, te, otherwise load from DB
            ...
            if (! isset($this->subjects[$subject->id])) {
                ...
                // Only update from DB if the local value is not set or we want to force refresh
    ...
    private function persistInscriptions()
    {
    ...
    public function saveAndConfirm()
    {
        ...
        $hasInscriptions = Inscription::where('configs_id', $this->inscription_id)
            ->where('user_id', $this->targetUser->id)
            ->whereIn('subject_id', function ($query) {
                $query->select('id')->from('subjects')->where('career_id', $this->career_id);
            })->exists();

        if (! $hasInscriptions) {
            $pdfFileName = "insc-{$this->targetUser->id}-{$this->career_id}-{$this->inscription_id}-.pdf";
            if (Storage::exists("private/inscriptions/{$pdfFileName}")) {
                Storage::delete("private/inscriptions/{$pdfFileName}");
            }
            $this->checkPdf();
            $this->success('Todas las selecciones eliminadas correctamente.');
    ...
    {
        return view('livewire.inscriptions.inscription-manager', [
            'items' => $this->items(),
