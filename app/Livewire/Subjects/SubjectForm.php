...
class SubjectForm extends Component
{
    ...
    public function save()
    {
        ...
        $this->validate([
            'data.id' => 'required|integer|unique:subjects,id,'.($this->original_id ?? 'NULL'),
            'data.career_id' => 'required|integer|exists:careers,id',
            'data.name' => 'required|string|max:255',
            'data.prerequisite' => 'nullable|string|max:255',
        ], [
            'data.id.required' => 'El ID de la materia es obligatorio.',
            'data.id.integer' => 'El ID de la materia debe ser un número entero.',
            'data.id.unique' => 'El ID de la materia ya está en uso.',
            'data.career_id.required' => 'La carrera es obligatoria.',
            'data.career_id.exists' => 'La carrera seleccionada no es válida.',
            'data.name.required' => 'El nombre de la materia es obligatorio.',
        ]);
