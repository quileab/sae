...
class ClassSessionForm extends Component
{
    ...
    public function mount($id = null)
    {
        ...
        if ($id !== null) {
            ...
            $this->data['class_number'] = ClassSession::where('subject_id', $this->subject_id)
    ...
    {
        // Security check for IDOR
        $this->authorizeSubject($this->data['subject_id']);
        if ($this->data['id']) {
            $existing = ClassSession::findOrFail($this->data['id']);
            $this->authorizeSubject($existing->subject_id);
        }

        ClassSession::updateOrCreate(['id' => $this->data['id']], $this->data);
        $this->success('Registro guardado');
        $this->redirect('/class-sessions?subject_id='.$this->subject_id);
    }
    ...
    public function delete()
    {
        ...
        if ($session) {
            // Security check for IDOR
            $this->authorizeSubject($session->subject_id);
    ...
    {
        return view('livewire.class_sessions.class-session-form');
    }
}
